<template>
  <div>
    <el-descriptions v-loading="loading" :column="1" border title="企业信息">
      <el-descriptions-item label="用户名">{{ profile.username || '-' }}</el-descriptions-item>
      <el-descriptions-item label="企业名称">{{ profile.enterpriseName || '-' }}</el-descriptions-item>
      <el-descriptions-item label="企业类型">{{ profile.typeName || '-' }}</el-descriptions-item>
...
const fetchProfile = async () => {
  try {
    const res = await request.get("/getEnterpriseProfile", {
      params: { enId: authStore.id },
    });
    if (res.data?.success !== false) {
      profile.username = res.data.username || "";
      profile.enterpriseName = res.data.enterpriseName || "";
      profile.typeName = res.data.typeName || "";
...
const handleSave = async () => {
  ...
  saving.value = true;
  try {
    const res = await request.put("/updateEnterpriseAddress", {
      enId: authStore.id,
...
const handlePwdSave = async () => {
  ...
  try {
    const res = await request.put("/updateEnterprisePassword", {
      enId: authStore.id,
